...
export default function LeadCardDetails({ 
  ...
  const handlePhoneChange = (val) => {
    // val is already expected to be '+94' + 9 digits from the input onChange
    setFormData(prev => ({ ...prev, phone: val }));

    // Limit digits for cross-check
    const digitsOnly = val.substring(3);

    // Database cross-checking: Check if contact number exists in customers database
    if (val.length === 12) {
      const match = customers.find(c => {
        const cPhone = c.phone?.replace(/[^\d+]/g, '');
        const targetPhone = val;
        return cPhone === targetPhone || (cPhone?.endsWith(digitsOnly) && digitsOnly.length >= 9);
      });
      if (match) {
  ...
  const analyzeCallRecording = async () => {
    ...
    try {
      ...
      // Specifically prompt for emoji-rich, non-technical Visual Scope of Work
      const instructions = `
        Use highly visual, non-technical language. 
        Add lots of relevant emojis to represent details. 
        Keep explanations simple and friendly so a client can easily grasp the details.
      `;
  ...
  const applyAudioAnalysisToScope = () => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      jobScope: audioAnalysisResult.scope || prev.jobScope,
  ...
  const saveInvoiceToDb = () => {
    ...
    onSaveInvoice({
      ...
      company: formData.company,
      date: invoiceDate,
      amount: formData.value * 0.75,
      totalValue: formData.value,
      type: 'Advance',
      status: 'Unpaid',
      aiDraft: formData.invoiceDraft
    });
    const updatedData = { ...formData, invoiceDate: invoiceDate };
    setFormData(updatedData);
    handleSaveLead(updatedData);
    toast.success(`Invoice ${invId} saved successfully in the database!`);
  };
  ...
  return (
    <div className="fixed inset-0 bg-surface-container-highest/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      {showCloseConfirm && (
        <div className="fixed inset-0 z-[60] bg-surface-container-highest/40 flex items-center justify-center">
      ...
      <div className="bg-surface-container rounded-3xl w-full max-w-6xl h-[88vh] flex flex-col shadow-[0_0_50px_rgba(0,218,243,0.25)] overflow-hidden border border-outline-variant animate-in fade-in duration-300">
        ...
        <div className="flex justify-between items-center px-8 py-5 flex-shrink-0 border-b border-outline-variant/50 bg-surface-container-low/50">
          <div>
            <h2 className="text-xl font-black text-on-surface">
              Lead pipeline: {formData.name || 'New Lead'}
            </h2>
            <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mt-1">
              ID: <span className="text-on-surface-variant font-mono">{lead.id}</span> • Pipeline Stage: <span className="text-primary font-black">{lead.stage}</span>
            </p>
          </div>
          <button 
            onClick={handleClose}
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto lg:grid lg:grid-cols-2 custom-scrollbar">
          
          {/* Left Column: Scope, Audio Analyzer, Pricing Engine, Client Details */}
          <div className="p-8 border-r border-outline-variant/50 space-y-8 h-full flex flex-col">
          ...
          {/* Scrollable Column 2: Quotation Manager (Quoted values, AI quote draft, AI invoice draft) */}
          <div className="h-full bg-surface-container-low/50 p-8 flex flex-col justify-start overflow-y-auto custom-scrollbar space-y-6">
            {/* OPTIMIZATION 1: Automated Pricing Engine at the Top */}
            <div className="p-6 bg-violet-50/50 rounded-2xl border border-violet-100/80 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className="p-2 bg-violet-600 text-on-surface rounded-xl">
                    <Calculator size={18} />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="flex bg-surface-container-low border border-outline-variant rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-primary/50">
                    ...
                    <input 
                      type="text"
                      name="phone"
                      value={formData.phone ? formData.phone.replace('+94', '') : ''}
                      onChange={(e) => {
                         const val = e.target.value.replace(/[^\d]/g, '').substring(0, 9);
                         handlePhoneChange('+94' + val);
                      }}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-surface-container p-5 rounded-2xl border border-outline-variant shadow-[0_4px_20px_rgba(0,218,243,0.05)] space-y-4">
                <div>
                  <label className="block text-[9px] uppercase font-bold text-on-surface-variant mb-1.5 tracking-widest">Quoted Contract Value (LKR)</label>
                  <input 
                    type="number"
                    name="value"
                ...
                <button 
                  ...
                  className={`w-full py-3 rounded-xl font-bold text-xs transition-all flex items-center justify-center space-x-2 ${
                    isGeneratingQuote 
                      ...
                      : formData.quotationGenerated && formData.quotationDraft
                        ? 'bg-surface-container text-on-surface-variant border border-outline-variant cursor-not-allowed'
                        : 'bg-primary text-on-primary hover:bg-primary/80 text-on-primary shadow-[0_4px_20px_rgba(0,218,243,0.05)] active:scale-[0.98]'
                  }`}
                >
                  {isGeneratingQuote ? 'Drafting with Gemini...' : formData.quotationGenerated ? 'Quotation Drafted' : 'AI Generate Quotation Draft'}
                </button>
            ...
            <div className="flex flex-wrap gap-3 mt-6">
              <button 
                onClick={() => {
                  ...
                  // Optionally close after save to give feedback it worked
                  onClose(); 
                }}
                className="flex-1 min-w-[140px] py-3.5 bg-surface-container-highest hover:bg-surface-container-high text-on-surface rounded-xl font-bold text-sm transition-all active:scale-[0.98] shadow-[0_4px_25px_rgba(0,218,243,0.1)] flex items-center justify-center space-x-2"
              >
                <Save size={16} />
              ...
              <button 
                ...
                className="flex-1 min-w-[140px] py-3.5 bg-secondary text-on-secondary hover:bg-secondary/80 text-on-primary rounded-xl font-bold text-sm transition-all active:scale-[0.98] shadow-[0_4px_25px_rgba(0,218,243,0.1)] flex items-center justify-center space-x-2"
              >
                <Check size={16} />
